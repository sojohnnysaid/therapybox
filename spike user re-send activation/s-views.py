...
class UsersLoginView(LoginView):
    ...
    def form_invalid(self, form):
        if 'username' in form.errors:
            account_activation_form_link = reverse_lazy('users:account_activation_request')
            account_not_activated_error_message = f'Account has not been activated! <a href="{account_activation_form_link}">Click here to resend activation link.</a>'
            messages.error(self.request, mark_safe(account_not_activated_error_message))
        else:
            messages.error(self.request, 'Login attempt failed. Please try again.')
